...
export class InMemoryBookRepository implements IBookRepository {
    ...
    GetByNumber(bookNumber: number): Promise<GetBookDto | undefined> {
        return new Promise((resolve, reject) => {
            const book = this.books.find(book => book.number === bookNumber);
            if (book === undefined) {
                reject;
                return;
            }
    ...
    Add(bookDto: AddBookDto): Promise<number> {
        const book = new Book(this.number++, bookDto.title, bookDto.author, bookDto.releaseYear, bookDto.price);
        return new Promise((resolve, reject) => {
    ...
    Update(bookDto: UpdateBookDto): Promise<void> {
        return new Promise((resolve, reject) => {
            const idx = this.books.findIndex(book => book.number === bookDto.number);
            if (idx === -1) {
                reject;
            }

            const book = this.books[idx];
            const updatedBook = new Book(book.number, bookDto.title, bookDto.author, bookDto.releaseYear, bookDto.price);
    ...
    Delete(bookNumber: number): Promise<void> {
        return new Promise((resolve, reject) => {
            const idx = this.books.findIndex(book => book.number === bookNumber);
            if (idx === -1) {
                reject;
            }
